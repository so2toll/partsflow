...
export const POST: APIRoute = async ({ request}) => {
    console.log("these the current params on supportdsfasdf endpoint",request)
    // const body = await request.json();
    const { cookie } = Object.fromEntries(new URL(request.url).searchParams);
console.log("the current cookie on server on support endpoint: ", cookie
)
    ...
    console.log("unsealedCookie on server on server dashboard endpoint: ", unsealedCookie);
    console.log("got to here on dashboard server login endpoint")
