...
export const post: APIRoute = async ({params, request}) => {
    console.log("these the current params on dashboard endpoint",params,request)
    // const body = await request.json();
    const { cookie } = Object.fromEntries(new URL(request.url).searchParams);
console.log("the current cookie on server on dashboard endpoint: ", cookie
)
    ...
    console.log("unsealedCookie on server on explorer post endpoint: ", unsealedCookie);
    console.log("got to here on explorer login endpoint")
...
async function main() {
    ...
    //     topic: 'persistent://public/default/mssy-topic',
    //     subscription: '3my-suaabscriptssion',
    //     subscriptionType: 'Exclusive',
    // });

    for (let i = 0; i < 5; i++) {
        const msg = await consumer.receive();
        console.log(msg.getData().toString(), "this is the message in consumer from explorer endpoint");
...
const response = {test:"131344", newTest:"54231 1 your done"};
