...
export const POST: APIRoute = async ({ request, cookies }) => {
  try {
    ...
    const updatedClient = ApiClientService.regenerateSecret(clientId);
    ...
    return new Response(JSON.stringify({
      ...
      data: {
        client: {
          ...
          secretRegeneratedAt: updatedClient.secretRegeneratedAt
        }
      }
    }), {
      status: 200,
    ...
    return new Response(JSON.stringify({
      message: 'error',
      error: error.message || 'Failed to regenerate client secret'
    }), {
      status: statusCode,
      headers: { 'Content-Type': 'application/json' }
    });
